...
export default class Tokenizer {
    ...
    private RESERVED_NEWLINE_REGEX;
    private RESERVED_PLAIN_REGEX;
    private WORD_REGEX;
    private STRING_REGEX;
    private OPEN_PAREN_REGEX;
    private CLOSE_PAREN_REGEX;
    private INDEXED_PLACEHOLDER_REGEX;
    private IDENT_NAMED_PLACEHOLDER_REGEX;
    private STRING_NAMED_PLACEHOLDER_REGEX;
    private RELINE_FUNCTION_PREFIX;
    /**
     * @param {Object} cfg
    ...
    constructor(cfg) {
        ...
        /**
         * 这个开头的时候，直接换行，用来匹配 OBJECT 这种类型
         */
        this.RELINE_FUNCTION_PREFIX = this.createRelineFunctionPrefix(cfg.relineFunctionPrefix);
    ...
    createRelineFunctionPrefix(words) {
        const reservedWordsPattern = words?.join("|").replace(/ /g, "\\s+");
        return new RegExp(`^(${reservedWordsPattern})\\b`, "i");
    }
    ...
    getNextToken(input, previousToken) {
        return this.getWhitespaceToken(input) ||
            ...
            this.getRelineFunctionPrefix(input) ||
    ...
    getRelineFunctionPrefix(input) {
        return this.getTokenOnFirstMatch({
            input,
            type: tokenTypes.RELINE_FUNCTION,
            regex: this.RELINE_FUNCTION_PREFIX
        });
    }
